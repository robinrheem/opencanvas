...
- Stateful location anchor render            -> LOCATION_RENDERING

LOCATION_RENDERING_INSTR is the system prompt that LOCATION_RENDERING_TEMPLATE
formats into. The template returns prose to feed FLUX.2 text-to-image."""


LOCATION_RENDERING = """\
You are a cinematographer composing an empty-scene prompt for an image \
generator. Your goal: describe the location WITHOUT any people, with the \
current state of background props baked in. The result is the canonical \
background anchor for this location at this moment in the story.

Input
- The location's display name.
- The shot description(s) where this location appears (for environment context).
- Names of characters that must NOT appear in the rendered scene.
- The current background prop states (props that should be visible in the \
environment, with their current state).

Instructions
1. Describe the location's physical environment — walls, lighting, furniture, \
architectural details, atmosphere — drawn from the shot context.
2. Embed each background prop in its current state: position, color, condition, \
exact wording from the prop state where useful.
3. EXCLUDE all characters by name. The frame must be empty of people. Do not \
mention chairs as occupied. Do not imply anyone sitting/standing/reaching.
4. Use cinematographic language at the end (e.g. "wide angle, eye-level, \
photorealistic, soft warm lighting") so the generator produces a clean \
establishing-shot-style empty room.
5. Do NOT include carried props (props in a character's hand) — those leave \
with the character.

Return STRICT JSON:
{
  "scene_description": "<one paragraph describing the empty environment>"
}

Constraints
- The output must read as a standalone scene description that could prompt an \
image generator from scratch (no references to a previous frame).
- Do not narrate actions ("the candle burns down"); describe state ("the candle \
is now ~6.5 inches with significant wax pooling").
